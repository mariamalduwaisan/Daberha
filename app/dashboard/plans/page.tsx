...
function StepIcon({ status }: { status: Step["status"] }) {
  if (status === "done")
    return <CheckCircle2 size={22} className="text-primary" fill="currentColor" strokeWidth={0} />;
  if (status === "active")
    return (
...
export default async function PlansPage() {
  ...
  const activePlanConfig = PLANS.find((p) => (progressMap[p.id] ?? 0) > 0) ?? null;
  const steps = activePlanConfig ? (PLAN_STEPS[activePlanConfig.id] ?? []) : [];
    ...
    <div className="flex flex-col min-h-screen bg-neutral pb-28">
      ...
      {activePlanConfig ? (
        <div className="px-5 py-5 space-y-5">
          {/* Active plan card */}
          <div className={`${activePlanConfig.iconBg.replace("/15", "")} rounded-2xl p-5 relative overflow-hidden`}>
            ...
            <h2 className="text-base font-extrabold text-white mt-2">{activePlanConfig.title}</h2>
            <p className="text-xs text-white/70 mt-1 leading-relaxed">{activePlanConfig.description}</p>
            <div className="mt-4">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-white/60">{doneCount} من {steps.length} خطوات</span>
                <span className="text-xs font-bold text-white">{progressMap[activePlanConfig.id] ?? 0}%</span>
              </div>
              <div className="h-1.5 bg-white/20 rounded-full overflow-hidden">
                <div className="h-full bg-white rounded-full transition-all" style={{ width: `${progressMap[activePlanConfig.id] ?? 0}%` }} />
              </div>
            </div>
          </div>
                ...
                return (
                  <div key={step.id} className="flex gap-3">
                    ...
                    <div className={`flex-1 mb-2 rounded-2xl p-4 ${
                      step.status === "active"
                        ? "bg-surface border border-primary shadow-sm shadow-primary/10"
                        : step.status === "done"
          ...
          {/* Skill focus */}
          <div>
            <h2 className="font-extrabold text-gray-900 mb-3">تركيز المهارات</h2>
        ...
        <div className="px-5 py-5 space-y-4">
          <p className="text-sm text-muted font-medium">اختر خطة لبدء مسارك التحضيري</p>
          <div className="space-y-2">
            {PLANS.map((plan) => (
              <div
                key={plan.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted mt-0.5 leading-snug line-clamp-1">{plan.description}</p>
      ...
      {/* Sticky bottom CTA */}
      {activePlanConfig && activeStep && (
        <div className="fixed bottom-16 inset-x-0 max-w-[430px] mx-auto px-5 pb-3 z-40">
